...
class MemberQuery extends Query {
  ...
  function execSearch($type, $word, $page) {
    ...
    if ($type == OBIB_SEARCH_BARCODE) {
      $col = "barcode_nmbr";
    } elseif ($type == OBIB_SEARCH_NAMEF) {
      $col = "first_name";
    } elseif ($type == OBIB_SEARCH_NAME) {
      $col = "last_name";
    } else if($type == OBIB_SEARCH_EMAIL) {
		$col = "email";
	}
  ...
  function _mkObj($array) {
    ...
    $mbr->setLastEmailDt($array["last_email_dt"]);
  ...
  function insert($mbr) {
    $sql = $this->mkSQL("insert into member "
                        . "(mbrid, barcode_nmbr, create_dt, last_change_dt, last_email_dt,"
                        . " last_change_userid, last_name, first_name, address, "
                        . " work_phone, email, classification) "
                        . "values (null, %Q, sysdate(), sysdate(), %Q, %N, "
                        . " %Q, %Q, %Q, %Q, %Q, %Q) ",
                        $mbr->getBarcodeNmbr(), $mbr->getLastEmailDt(), $mbr->getLastChangeUserid(),
                        $mbr->getLastName(), $mbr->getFirstName(),
                        $mbr->getAddress(),  $mbr->getWorkPhone(), 
  ...
  function update($mbr) {
    $sql = $this->mkSQL("update member set "
                        . " last_change_dt = sysdate(), last_email_dt=%Q, last_change_userid=%N, "
                        . " barcode_nmbr=%Q,  last_name=%Q,  first_name=%Q, "
                        . " address=%Q, work_phone=%Q, "
                        . " email=%Q, classification=%Q "
                        . "where mbrid=%N",
                        $mbr->getLastEmailDt(),
                        $mbr->getLastChangeUserid(), $mbr->getBarcodeNmbr(),
                        $mbr->getLastName(), $mbr->getFirstName(),
                        $mbr->getAddress(),  $mbr->getWorkPhone(), $mbr->getEmail(),
