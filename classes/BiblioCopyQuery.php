...
class BiblioCopyQuery extends Query {
  ...
  function _mkObj($array) {
    ...
    $copy->setLastMailDt($array["last_mail_dt"]);
  ...
  function insert($copy) {
    ...
    $sql = $this->mkSQL("insert into biblio_copy values (%N"
                        . ",null, now(), sysdate(), %Q, %Q, %Q, sysdate(), ",
  ...
  function update($copy) {
    ...
    $sql = $this->mkSQL("update biblio_copy set "
                        . "copy_desc=%Q, barcode_nmbr=%Q, last_mail_dt=%Q "
                        . "where bibid=%N and copyid=%N ",
                        $copy->getCopyDesc(), $copy->getBarcodeNmbr(), $copy->getLastMailDt(),
  ...
  function updatebc($copy, $refbibid, $refcopyid) {
    $sql = $this->mkSQL("update biblio_copy set "
                        . "copy_desc=%Q, barcode_nmbr=%Q, last_mail_dt=%Q, "
                        . "bibid=%N, copyid=%N "
                        . "where bibid=%N and copyid=%N ",
                        $copy->getCopyDesc(), $copy->getBarcodeNmbr(), $copy->getLastMailDt(),
                        $copy->getBibid(), $copy->getCopyid(), $refbibid, $refcopyid);
    $ret = $this->_query($sql, $this->_loc->getText("biblioCopyQueryErr5"));
    if (!$ret) {
      return $ret;
    }
    $this->setCustomFields($refbibid, $refcopyid, $copy->_custom);
    $this->delete($refbibid, $refcopyid);
    return $ret;
  }
