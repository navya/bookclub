...
class TableFuncs {
  ...
  function biblio_edit_link($col, $row, $params) {
	    global $tab;	# FIXME - get rid of $tab
	      $url = '../catalog/biblio_edit.php?bibid='.HURL($row['bibid']);
	      if ($tab != 'opac') {
		          $url .= '&amp;tab=cataloging';
			    } else {
				        $url .= '&amp;tab=opac';
  ...
  function member_link1($col, $row, $params) {
    $url = '../circ/mbr_view.php?mbrid='.HURL($row['mbrid1']);
    return TableFuncs::_link_common($col, $row, $params, $url, 'mbrid1');
  }
