 ...
 */
  ...
  $filename = '../autoemail/'.$_REQUEST["name"].'.php';
  if (!is_readable($filename)) {
    $filename = '../autoemail/default/'.$_REQUEST["name"].'.php';
  }
  assert('is_readable($filename)');
  $classname = 'Autoemail_'.$_REQUEST["name"];
  ...
  if (isset($_REQUEST['rpt'])) {
    ...
    $rpt = new Iter;  # Some autoemail don't need a report.
  }
  assert('$rpt != NULL');
  
  // Rendering a large autoemail can take a while.
  set_time_limit(90);
  ...
  $nav = "autoemailparams";
  $focus_form_name = "autoemailparamform";
...
<form name="autoemailparamform" method="GET" action="../shared/autoemail.php">
