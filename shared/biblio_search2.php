 ...
 */
  ...
  require_once("../classes/BiblioSearchQuery2.php");
...
# limit search results to collections? (array handling similar to checkin.php)
  if (!$collecs) {
    foreach($HTTP_POST_VARS as $key => $value) {
      if ($value == "collec") {
        parse_str($key,$output);
        $collecs[] = $output["collec"];
      }
    }
  }
  if ($colleccount == count($collecs)) {$collecs = "";}
# limit search results to materials? (array handling similar to checkin.php)
  if (!$materials) {
    foreach($HTTP_POST_VARS as $key => $value) {
      if ($value == "material") {
        parse_str($key,$output);
  ...
  if ($matcount == count($materials)) {$materials = "";}
  #****************************************************************************
  #*  Search database
  #****************************************************************************
  $biblioQ = new BiblioSearchQuery2();
  ...
  if (!$biblioQ->search($sType,$words,$currentPageNmbr,$sortBy,$opacFlg,$collecs,$materials)) {
    $biblioQ->close();
    displayErrorPage($biblioQ);
  }
...
<!--**************************************************************************
    ...
    ************************************************************************** -->
<form name="changePageForm" method="POST" action="../shared/biblio_search2.php">
...
<?php 
  if ($collecs) {
    for ($i = 0; $i < count($collecs); $i++) {
      echo "  <input type=\"hidden\" name=\"collec=$collecs[$i]\" value=\"collec\">\n";
      } ?>
      <input type="hidden" name="colleccount" value="<?php echo $_POST["colleccount"]; ?>">
<?php } ?>
<?php 
  if ($materials) {
    for ($j = 0; $j < count($materials); $j++) {
      echo "  <input type=\"hidden\" name=\"material=$materials[$j]\" value=\"material\">\n";
      } ?>
      <input type="hidden" name="matcount" value="<?php echo $_POST["matcount"]; ?>">
<?php } ?>
</form>
...
<!--**************************************************************************
    ...
    ************************************************************************** -->
<?php
  if ($collecs){echo "Searched in selected Collections"."<br>";} 
